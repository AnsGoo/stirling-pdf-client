...
class InfoApi:
    ...
    def get_status(self) -> Status:
        ...
        return Status(version=status_data.get('version', ''), status=status_data.get('status', ''))
    
    def get_load(self, endpoint: Optional[str] = None) -> int:
        url = '/api/v1/info/load'
        resp:Response = self.__client.request(method='GET', url=url, params={"endpoint":endpoint})
        # 将JSON响应转换为Status类型
        status_data = resp.json()
        return status_data
    ...
    def get_load_all(self, endpoint: Optional[str] = None) -> List[LoadCount]:
        ...
        return list(map(lambda el: LoadCount(endpoint=el.get('endpoint',""), count=el.get("count",0)), data))
