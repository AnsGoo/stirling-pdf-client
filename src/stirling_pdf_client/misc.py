...
class MiscApi:
    ...
    def update_metadata(self, out_path: Path, file_input: Optional[Path],
        fileId: Optional[str] = None,delete_all:Optional[bool]=False, options: Optional[UpdateMetadataOptions] = None) -> str:
        ...
        if file:
            file.close()
        return resp.text
    def unlock_pdf_forms(self, out_path: Path, file_input: Optional[Path], fileId: Optional[str] = None) -> str:
    ...
    def scanner_effect(self, out_path: Path, file_input: Path, quality: Literal["low", "medium", "high"] = "hight", rotation:Literal["none","slight","moderate","severe"] = "none", options: Optional[ScannerEffectOption] = {
         "border": 20,
        "rotate": 0,
        "rotate_variance": 0,
        "brightness": 1,
